	...
	public function _edit($gallery_id, $category_id, $image_id, $name, $published, $title, $description, $category_name, $category_title, $category_image, $category_icon)
	{
		...
		$gallery_table = $this	->table
								->add_columns([
									[
										'content' => function($data){
											return '<a class="thumbnail thumbnail-link" data-toggle="tooltip" title="'.$this->lang('Visualiser').'" data-image="'.$this->model2('file', $data['file_id'])->path().'" data-title="'.$data['title'].'" data-description="'.$data['description'].'"><img style="max-width: 80px;" src="'.$this->model2('file', $data['thumbnail_file_id'])->path().'" alt="" /></a>';
										},
										'size'    => TRUE
									],
									[
			...
			$this->col(
				$this	->panel()
						...
						->body(function($data) use ($image_id, $title, $description, $thumbnail_file_id){
							return '<a class="thumbnail thumbnail-link no-margin" data-toggle="tooltip" title="'.$this->lang('Visualiser').'" data-image-id="'.$image_id.'" data-image-title="'.url_title($title).'" data-image-description="'.$description.'"><img src="'.$this->model2('file', $thumbnail_file_id)->path().'" alt="" /></a>';
						})
