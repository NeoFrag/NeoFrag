<img class="img-responsive" src="<?php echo $this->model2('file', $file_id)->path() ?>" alt="" />
	...
	<div class="row">
		...
		<div class="col-md-2">
			<a class="thumbnail<?php echo ($image_id == $vignette['image_id']) ? ' active' : ''  ?>" href="<?php echo url('gallery/image/'.$vignette['image_id'].'/'.url_title($vignette['title'])) ?>">
				<img class="img-responsive" src="<?php echo $this->model2('file', $vignette['thumbnail_file_id'])->path() ?>" alt="" />
			</a>
		</div>
		<?php endforeach ?>
	</div>
