...
class Index extends Controller_Module
{
	public function index($events)
	{
		...
		foreach ($events as $event)
		{
			...
			{
				$panels[] = $this	->panel()
									->heading('<a href="'.url('events/'.$event['event_id'].'/'.url_title($event['title'])).'">'.$event['title'].'</a>'.(!empty($data['match']) ? '<div class="pull-right">'.($data['match']['game']['icon_id'] ? '<img src="'.$this->model2('file', $data['match']['game']['icon_id'])->path().'" alt="" />' : icon('fa-gamepad')).' '.$data['match']['game']['title'].'</div>' : ''), $icon)
	...
	public function _event($event_id, $title, $type_id, $date, $date_end, $description, $private_description, $location, $image_id, $published, $type, $mode_id, $webtv, $website, $mode_title)
	{
				...
				->add_columns([
					[
						'content' => function($data){
							return $this->model2('user', $data['user_id'])->avatar();
						},
						'size'    => TRUE
					],
					[
		...
		return [
			$this->_filters(),
			$this	->panel()
					->heading('<a href="'.url('events/'.$event_id.'/'.url_title($title)).'">'.$title.'</a>'.(!empty($match) ? '<div class="pull-right">'.($match['game']['icon_id'] ? '<img src="'.$this->model2('file', $match['game']['icon_id'])->path().'" alt="" />' : icon('fa-gamepad')).' '.$match['game']['title'].'</div>' : ''), $type == 1 ? 'fa-crosshairs' : 'fa-calendar-o')
