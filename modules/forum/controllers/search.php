...
class m_forum_c_search extends Controller_Module
{
	public $name = 'Forum';
	...
	public function search($keywords, $not_keywords)
	{
		...
		{
			array_push($args, 't.title LIKE',    '%'.$keyword.'%', 'OR'
		...
		if ($not_keywords)
		{
			...
			{
				array_push($args, 't.title NOT LIKE',    '%'.$keyword.'%', 'AND'
								, 'm.message NOT LIKE',  '%'.$keyword.'%', 'AND');
			}
