<div class="table-responsive">
	<table class="table table-hover">
		...
		<tbody class="forum-content">
			...
			<tr>
				...
				<td class="col-md-6">
					<?php
						...
						{
							echo '<div class="pull-right">'.$loader->pagination->display(url('forum/topic/'.$topic['topic_id'].'/'.url_title($topic['title'])), ceil($topic['count_messages'] / $NeoFrag->config->forum_messages_per_page), 'xs').'</div>';
						}
					?>
					<h4 class="no-margin"><a href="<?php echo url('forum/topic/'.$topic['topic_id'].'/'.url_title($topic['title']).'.html'); ?>"><?php echo $topic['title']; ?></a></h4>
					<div><?php echo icon('fa-user').' '.($topic['user_id'] ? $NeoFrag->user->link($topic['user_id'], $topic['username']) : '<i>'.i18n('guest').'</i>').' '.icon('fa-clock-o').' '.time_span($topic['date']); ?></div>
				</td>
				<td>
					<?php echo i18n('messages', $topic['count_messages'], $topic['count_messages']); ?><br />
					<?php echo i18n('views', $topic['views'], $topic['views']); ?>
				</td>
				<td>
					...
					<div><a href="<?php echo url('forum/topic/'.$topic['topic_id'].'/'.url_title($topic['title']).($topic['count_messages'] > $NeoFrag->config->forum_messages_per_page ? '/page/'.ceil($topic['count_messages'] / $NeoFrag->config->forum_messages_per_page) : '').'.html#message_'.$topic['last_message_id']); ?>"><?php echo icon('fa-comment-o').' '.str_shortener(strip_tags(str_replace('<br />', ' ', bbcode($topic['message']))), 35); ?></a></div>
						<div><?php echo icon('fa-user').' '.($topic['last_user_id'] ? $NeoFrag->user->link($topic['last_user_id'], $topic['last_username']) : '<i>'.i18n('guest').'</i>').' '.icon('fa-clock-o').' '.time_span($topic['last_message_date']); ?></div>
					<?php else: ?>
						<?php echo i18n('no_answer'); ?>
					<?php endif; ?>
				</td>
			</tr>
			<?php endforeach; ?>
			...
			<tr>
				<td colspan="4" class="text-center"><h4><?php echo i18n('no_message'); ?></h4></td>
			</tr>
			<?php endif; ?>
		</tbody>
	</table>
