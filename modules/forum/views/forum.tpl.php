	...
	<tbody class="forum-content">
		...
		<tr>
			...
			<td class="col-3">
				...
				<div><small><?php echo icon('fa-user').' '.($topic['last_user_id'] ? $this->user->link($topic['last_user_id'], $topic['last_username']) : '<i>'.$this->lang('Visiteur').'</i>').' '.icon('fa-clock-o').' '.time_span($topic['last_message_date']) ?></small></div>
				<?php else: ?>
