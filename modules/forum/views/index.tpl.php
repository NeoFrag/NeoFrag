			...
			<tr<?php if ($this->url->admin) echo ' data-forum-id="'.$forum['forum_id'].'"'; ?>>
					...
					if (!empty($forum['subforums']) || $this->url->admin):
						...
						foreach ($forum['subforums'] as $subforum):
							echo '<li'.($this->url->admin ? ' data-forum-id="'.$subforum['forum_id'].'"' : '').'>'.
									($this->url->admin ? '<div class="pull-right">'.$this->button_update('admin/forum/'.$subforum['forum_id'].'/'.url_title($subforum['title']).'.html').' '.$this->button_delete('admin/forum/delete/'.$subforum['forum_id'].'/'.url_title($subforum['title']).'.html').'</div>' : '')
				...
				<td>
					...
					<?php if ($forum['last_title']): ?>
						<div><a href="<?php echo url('forum/topic/'.$forum['topic_id'].'/'.url_title($forum['last_title']).($forum['last_count_messages'] > $this->config->forum_messages_per_page ? '/page/'.ceil($forum['last_count_messages'] / $this->config->forum_messages_per_page) : '').'.html#message_'.$forum['last_message_id']); ?>"><?php echo icon('fa-comment-o').' '.str_shortener($forum['last_title'], 40); ?></a></div>
