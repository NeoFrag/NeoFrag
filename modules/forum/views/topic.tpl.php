...
<table class="table">
	...
	<tbody class="forum-content">
		<tr>
			<td class="col-md-3">
				<?php echo NeoFrag::loader()->module->get_profile($data['user_id'], $profile); ?>
			</td>
			<td class="text-left col-md-9">
				<div class="padding-top">
					...
					<a name="message_<?php echo $data['message_id']; ?>"></a><?php echo icon('fa-clock-o').' '.time_span($data['date']).' '.($data['last_message_read'] && $data['date'] <= $data['last_message_read'] ? icon('fa-comment-o').' '.$this->lang('message_read') : icon('fa-comment').' '.$this->lang('message_unread')); ?>
				</div>
				<hr />
