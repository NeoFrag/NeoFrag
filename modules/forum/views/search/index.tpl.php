<p class="pull-right"><?php echo icon('fa-comments').' <a href="'.url('forum/'.$data['forum_id'].'/'.url_title($data['title'])).'">'.$data['title'].'</a> '.icon('fa-user').' '.($data['user_id'] ? $this->user->link($data['user_id'], $data['username']) : '<i>'.$this->lang('Visiteur').'</i>').' '.icon('fa-clock-o').' '.time_span($data['date']) ?></p>
