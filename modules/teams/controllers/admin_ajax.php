...
class m_teams_c_admin_ajax extends Controller
{
	public function sort($team_id, $position)
	{
		...
		foreach (array_merge(array_slice($teams, 0, $position, TRUE), array($team_id), array_slice($teams, $position, NULL, TRUE)) as $order => $team_id)
		{
	...
	public function _roles_sort($role_id, $position)
	{
		...
		foreach (array_merge(array_slice($roles, 0, $position, TRUE), array($role_id), array_slice($roles, $position, NULL, TRUE)) as $order => $role_id)
		{
