...
class Index extends Controller_Module
{
	public function index()
	{
		...
		foreach ($this->model()->get_teams() as $team)
		{
			...
			if ($team['image_id'])
			{
				$panel->body('<a href="'.url('teams/'.$team['team_id'].'/'.$team['name']).'"><img class="img-responsive" src="'.$this->model2('file', $team['image_id'])->path().'" alt="" /></a>', FALSE);
			}
						...
						->add_columns([
							...
							[
								'content' => function($data){
									...
									{
										return '<img src="'.$this->model2('file', $data['match']['opponent']['image_id'])->path().'" style="max-height: 35px; max-width: 50px;" alt="" />';
									}
									else
									{
										return '';
