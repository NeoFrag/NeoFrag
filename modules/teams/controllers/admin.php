		...
		$teams = $this	->table
						->add_columns([
							...
							[
								...
								'content' => function($data){
									return '<a href="'.url('teams/'.$data['team_id'].'/'.$data['name']).'"><img src="'.$this->model2('file', $data['icon_id'])->path().'" alt="" /> '.$data['title'].'</a>';
								}
							],
							[
								'title'   => $this->lang('Jeux'),
								'content' => function($data){
									return '<a href="'.url('admin/games/'.$data['team_id'].'/'.$data['game']).'"><img src="'.$this->model2('file', $data['game_icon'])->path().'" alt="" /> '.$data['game_title'].'</a>';
								}
							],
							[
