...
<div class="about-content">
	<?php if ($settings['display_teamname'] == 'oui'): ?>
		...
		<ul class="list-inline <?php echo $settings['teamname_align'] ? $settings['teamname_align'] : 'text-left' ?>"<?php echo $settings['style_title'] ? ' style="color: '.$settings['style_title'].'"' : '' ?>>
			<?php if ($settings['display_type'] == 'oui'): ?><li class="list-inline-item"><?php echo icon('fa-institution').' '.$this->config->nf_team_type ?></li><?php endif ?>
			<?php if ($settings['display_date'] == 'oui'): ?><li class="list-inline-item"><?php echo icon('fa-calendar').' '.timetostr('%e %b %Y', $this->config->nf_team_creation) ?></li><?php endif ?>
		</ul>
		<?php endif ?>
	<?php endif ?>
