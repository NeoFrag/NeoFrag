	...
	$('body').on('keyup', '.table-search', function(){
		...
		request[id] = $.ajax({
			...
			if (typeof data.table != 'undefined'){
				$table.show().html(data.table).next('.card-body').remove();
				$('body').trigger('nf.load');
			}
			else if ($table.is(':visible')) {
				$table.hide().after('<div class="card-body"><?php echo $this->lang('Aucun résultat ne correspond à la recherche') ?></div>');
			}
