		...
		if (NEOFRAG_LIVE_EDITOR)
		{
			if (NEOFRAG_LIVE_EDITOR & NEOFRAG_ZONES)
			{
				$output = '	<div class="pull-right">
							...
							<h3>'.(!empty($this->output->theme()->info()->zones[$zone_id]) ? $this->output->theme()->lang($this->output->theme()->info()->zones[$zone_id], NULL) : $this->lang('Zone #%d', $zone_id)).' <div class="btn-group"><button type="button" class="btn btn-xs btn-success live-editor-add-row" data-toggle="tooltip" data-container="body" title="'.$this->lang('Nouveau Row').'">'.icon('fa-plus').'</button></div></h3>'.
							$output;
			}
